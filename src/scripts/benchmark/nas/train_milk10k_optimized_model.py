...
def pipeline(dataset, num_metadata_features, num_epochs, batch_size, device, multimodel_config, k_folds, num_classes, model_name, num_heads, common_dim, text_model_encoder, unfreeze_weights, attention_mecanism, results_folder_path, num_workers=5, persistent_workers=False):
    try:    
        ...
        groups = dataset.metadata["lesion_id"].values   # agrupa por paciente
        ...
        for fold, (train_idx, val_idx) in enumerate(
            ...
            train_dataset = type(dataset)(
                ...
                type_of_problem = dataset.type_of_problem,
        ...
        raise ValueError(f"Erro ao realizar o pipeline: {e}\n")
                ...
                try:
                    if (text_model_encoder in ['one-hot-encoder', "tab-transformer"]):
                        dataset = skinLesionDatasetsMILK10K.SkinLesionDataset(                        
                            metadata_file="/data/MILK10K/MILK10k_Training_Metadata.csv",
                            train_ground_truth="/data/MILK10K/MILK10k_Training_GroundTruth.csv",
